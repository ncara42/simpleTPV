...
test('un ADMIN entra y ve las pestañas de administración', async ({ page }) => {
  await page.getByTestId('login-email').fill('admin@org1.test');
  await page.getByTestId('login-password').fill('password123');
  await page.getByTestId('login-submit').click();

  await expect(page.getByTestId('tab-families')).toBeVisible({ timeout: 10000 });
...
test('un CLERK no entra: ve acceso restringido', async ({ page }) => {
  ...
  await expect(page.getByTestId('tab-families')).toHaveCount(0);
});
