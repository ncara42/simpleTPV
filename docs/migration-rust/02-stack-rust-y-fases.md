...
1. 🔶 **task_local! vs paso explícito del tenant** — task*local no se hereda en `tokio::spawn`; un olvido = fuga potencial. \_Recomendación:* preferir **pasar `organization_id` explícito** a la capa `db` (o ambos: task_local para ergonomía + assert de coherencia). Decidir en Fase 0.
