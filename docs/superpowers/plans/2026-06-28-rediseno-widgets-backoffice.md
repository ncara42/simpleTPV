...
Trabajo integrado en **`main`** (2026-06-29). **Hecho: tandas 0, 1, 2, 3, 5, 6, 7, 8, 9, 10** (33 widgets
nuevos + 2 clásicos conservados, 11 categorías de la galería pobladas). Cada tanda con su test (datos mock

- paridad) y la puerta verde (typecheck + lint + unit + build). Las Tandas 8, 9 y 10 además verificadas por
  captura propia vs. el handoff. **Tanda 4 DEFERIDA** (faltan datos). **Pendiente: tanda 11** (Especializados).
