...
Rama `origin/feat/rediseno-widgets-backoffice` (continuación de la Tanda 8 en `feat/widgets-tandas-finales`).
**Hecho: tandas 0, 1, 2, 3, 5, 6, 7, 8, 9** (30 widgets nuevos + 2 clásicos conservados, 10 categorías de
la galería pobladas). Cada tanda con su test (datos mock + paridad) y la puerta verde (typecheck + lint +
unit + build). Las Tandas 8 y 9 además verificadas por captura propia vs. el handoff. **Tanda 4 DEFERIDA**
(faltan datos). **Pendiente: tandas 10, 11** (ver roadmap; varios widgets diferidos por falta de
endpoint, anotados en cada tanda).

> ⚠️ **Divergencia con `main` (2026-06-29):** `main` integró trabajo de widgets más nuevo (p. ej.
> `graf-hour-area`, `graf-store-bars`, renombrado `lista-mix`→`SalesMix`) que NO está en esta rama; y esta
> rama tiene las tandas 8–9 que NO están en `main`. Al integrar habrá que conciliar `panels/index.tsx`,
> `registry.ts`, `dashboard-layout.ts` y `gallery-catalog.tsx` (cambios aditivos, conflictos resolubles).
