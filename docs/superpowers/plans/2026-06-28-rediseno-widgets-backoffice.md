...
- **Tanda 4 · 04 Más exploraciones (4)** ✅ — desbloqueada añadiendo **4 endpoints backend honestos**
  (`crates/domain/src/dashboard` + `crates/http`): `sales-by-payment` (reparto por método de pago),
  `recent-sales` (últimas ventas), `sales-goal` (en curso · objetivo = periodo anterior completo ·
  proyección por ritmo, vía `previous_full_period`/`period_full_end`) y `cumulative-month` (acumulado
  diario del mes vs. el anterior + proyección por la forma del mes anterior). Widgets en
  `panels/exploraciones.tsx`: `exp-objetivo` (BulletMeter), `exp-metodos-pago` (DonutStat),
  `exp-tickets-recientes` (ActivityFeed) y `exp-acumulado-mes` (ProjectionArea). Tests de integración
  contra Postgres (4) + tests de panel (4) + paridad de catálogo. **Sin datos fabricados**: si no hay
  periodo anterior con ventas, el objetivo es 0 y la molécula muestra «sin datos». Estos endpoints
  también DESBLOQUEAN los 2 minis diferidos de la Tanda 8 (_Stacked · pago_ y _Bullet · objetivo_).
