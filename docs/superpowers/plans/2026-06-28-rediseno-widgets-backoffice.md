...
- **Tanda 2 · 02 Gráficas (3)** ⬜ — `dash-hour` ✅ (área horaria) y `dash-bars` ✅ (ventas por tienda)
  ya existen; **nuevo**: _graf-heatmap_ (HeatStrip, `getSalesByHourOnDay`/día·hora).
- **Tanda 3 · 03 Listas (3)** ⬜ — _lista-familia_ (ShareBar, `getSalesByFamily`),
  _lista-rankings_ (Leaderboard, `getProductRankings`), _lista-mix_ (ShareBar apilado / Treemap).
- **Tanda 4 · 04 Más exploraciones (4)** ⬜ — _exp-acumulado-mes_ (ProjectionArea, `getSalesToday`),
  _exp-objetivo_ (BulletMeter), _exp-metodos-pago_ (DonutStat — ¿endpoint?), _exp-tickets-recientes_
  (ActivityFeed / lista, `getSalesToday`).
- **Tanda 5 · 05 Compactos (5)** ⬜ — _cmp-ribbon_ (RibbonStat), _cmp-treemap_ (Treemap, familia),
  _cmp-donut_ (DonutStat), _cmp-leaderboard_ (Leaderboard, empleados), _cmp-hero_ (HeroFigure).
- **Tanda 6 · 06 Diagnóstico (1)** ⬜ — _diag-actividad_ (ActivityFeed, `listAlerts`).
- **Tanda 7 · 07 KPIs · más formatos (4)** ⬜ — _kpi-dual_ (KpiDual), _kpi-area_ (KpiStat + SparkArea),
  _kpi-alerta_ (KpiStat tono danger, `getStockoutKpis`), _kpi-7dias_ (KpiStat + SparkBars 7d).
