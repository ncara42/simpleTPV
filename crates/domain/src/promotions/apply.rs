...
pub fn best_promotions(promos: &[Promotion], input: &MatchInput) -> PromoOutcome {
    ...
    for (idx, line) in input.lines.iter().enumerate() {
        ...
        for p in &live {
            if !matches!(
                p.applies_to,
                PromoAppliesTo::Product | PromoAppliesTo::Family
            ) {
                continue;
            }
...
mod tests {
    ...
    fn ticket_min_ticket_aplica_porcentaje() {
        ...
        let skip = HashSet::new();
        let out = best_promotions(
            &[pr.clone()],
            &input(&lines, Uuid::new_v4(), now(1, 10, 0), &skip),
        );
    ...
    fn happy_hour_dentro_de_franja_aplica() {
        ...
        let out = best_promotions(
            &[pr.clone()],
            &input(&lines, Uuid::new_v4(), now(5, 18, 30), &skip),
        );
    ...
    fn franja_que_cruza_medianoche() {
        ...
        assert_eq!(
            best_promotions(
                &[pr.clone()],
                &input(&lines, Uuid::new_v4(), now(5, 23, 30), &skip)
            )
            .lines
            .len(),
            1
        ...
        assert_eq!(
            best_promotions(
                &[pr.clone()],
                &input(&lines, Uuid::new_v4(), now(5, 1, 0), &skip)
            )
            .lines
            .len(),
            1
        ...
        // 12:00 fuera.
        assert!(
            best_promotions(&[pr], &input(&lines, Uuid::new_v4(), now(5, 12, 0), &skip))
                .lines
                .is_empty()
        );
    }
    ...
    fn weekday_fuera_de_lista_no_aplica() {
        ...
        let skip = HashSet::new();
        // Miércoles (3) → fuera.
        assert!(best_promotions(
            &[pr.clone()],
            &input(&lines, Uuid::new_v4(), now(3, 12, 0), &skip)
        )
        .lines
        .is_empty());
        // Sábado (6) → aplica.
        assert_eq!(
            best_promotions(&[pr], &input(&lines, Uuid::new_v4(), now(6, 12, 0), &skip))
                .lines
                .len(),
            1
        );
    ...
    fn promo_acotada_a_otra_tienda_no_aplica() {
        ...
        let skip = HashSet::new();
        assert!(
            best_promotions(&[pr.clone()], &input(&lines, store_a, now(1, 12, 0), &skip))
                .lines
                .is_empty()
        );
        assert_eq!(
            best_promotions(&[pr], &input(&lines, store_b, now(1, 12, 0), &skip))
                .lines
                .len(),
            1
        );
    ...
    fn gana_la_mejor_entre_dos_promos_de_linea() {
        ...
        let out = best_promotions(
            &[a, b.clone()],
            &input(&lines, Uuid::new_v4(), now(1, 12, 0), &skip),
        );
    ...
    fn empate_de_descuento_gana_mayor_prioridad() {
        ...
        assert_eq!(
            out.lines[0].promotion_id, b.id,
            "empate → mayor prioridad (primera)"
        );
    }
    ...
    fn promo_skipped_se_ignora() {
        ...
        assert!(
            out.lines.is_empty(),
            "la promo quitada por el cajero se ignora"
        );
    }
