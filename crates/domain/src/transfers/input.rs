...
mod tests {
    ...
    fn acepta_jpeg_png_webp_y_deriva_mime() {
        assert_eq!(
            att("data:image/jpeg;base64,AAAA").validate(),
            Ok("image/jpeg")
        );
        assert_eq!(
            att("data:image/png;base64,AAAA").validate(),
            Ok("image/png")
        );
        assert_eq!(
            att("data:image/webp;base64,AAAA").validate(),
            Ok("image/webp")
        );
    }
    ...
    fn rechaza_mime_no_permitido_vacio_y_no_base64() {
        ...
        assert_eq!(
            att("data:image/gif;base64,AAAA").validate(),
            Err(AppError::BadRequest)
        );
        // payload vacío
        assert_eq!(
            att("data:image/png;base64,").validate(),
            Err(AppError::BadRequest)
        );
    ...
    fn rechaza_por_encima_del_tope() {
        let huge = format!(
            "data:image/jpeg;base64,{}",
            "A".repeat(MAX_IMAGE_DATA_URL_LEN)
        );
